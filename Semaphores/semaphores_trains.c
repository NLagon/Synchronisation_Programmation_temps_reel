...
void train1_deplacement(int id, int tempsDepla_train1,char*train1){ // fonction représentant le déplacement du train 1 avec une zone critique permettant l'accès 
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		if(((train1[i]==gare_t2D && train1[i+1]==gare_t2A)&&(train1[i]==gare_t3D && train1[i+1]==gare_t3A))){	
			...
			sem_wait(train_sem); // Attente d'un jeton qui permettra l'entrée du train sur la liaison
			...
			sem_post(train_sem);// on donne un jeton pour l'exécution du thread qui est en attente

			sem_wait(train_sem);// on reprend la main lorsque tous les threads ont accédé à la zone critique juste avant
			printf("Le train %d a quitté la liaison %c--%c\n", id,train1[i],train1[i+1]);
			sem_post(train_sem);// on donne un jeton pour l'exécution du thread
		}
		...
		else if ((train1[i]==gare_t2D && train1[i+1]==gare_t2A)||(train1[i]==gare_t3D && train1[i+1]==gare_t3A))
		{
			...
			sem_wait(train_sem);// on reprend la main lorsque tous les threads ont accédé à la zone critique juste avant
			// zone critique permettant la sortie dans l'ordre d'arrivée
			printf("Le train %d a quitté la liaison %c--%c\n", id,train1[i],train1[i+1]);
			sem_post(train_sem); // on donne un jetion pour l'exécution du thread suivant
		}


		// Si le train1 arrive dans le sens contraire aux  train2 ou train3, empêche le croisement 
		...
		else if ((train1[i]==gare_t2A && gare_t2D==train1[i+1])||(train1[i]==gare_t3A && gare_t3D==train1[i+1]))
		{
			...
			sem_wait(train_sem);// Attente du jeton pour permettre l'entrée du train sur la liaison
			...
			sem_post(train_sem); // on donne un jeton pour l'exécution du thread suivant
...
void train2_deplacement(int id,int tempsDepla_train2,char*train2){	// fonction représentant le déplacement du train 2
	...
	for (int i = 0; i < 5; ++i)
	{
		...
		// Si le train2 arrive dans le sens contraire aux  train1 ou train3, empêche le croisement 
	...
	vt2+=difftime(fin_t2,debut_t2);									   // et obtenir le temps complet des 1000 trajets effectués par le train 1	
...
void train3_deplacement(int id,int tempsDepla_train3,char*train3){	// fonction représentant le déplacement du train 2
	...
	vt3+=difftime(fin_t3,debut_t3);									   // et obtenir le temps complet des 1000 trajets effectués par le train 1	
...
void* train(void *arg) {
	...
	for (i = 1; i <=1000; i++) {
		...
		int tempsDepla_train1=rand() % 3 +1; // temps de déplacement des trains 
...
int main() {
	...
	FILE* fichier=NULL; // ouverture d'un fichier dans lequel on écrit les résultats du banc de test
 	fichier=fopen("banc_de_test2.txt","w");
    ...
    train_sem = sem_open("turnA", O_CREAT, S_IRUSR | S_IWUSR, 0);

    if (train_sem == SEM_FAILED ) {
    ...
    sem_post(train_sem);// Donne un jeton qui va être accepté pour entrer dans la zone critique d'un thread
